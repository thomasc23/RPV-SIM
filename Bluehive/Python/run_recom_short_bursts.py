...
# global checkpoint flag set by SLURM signal
_SHOULD_FLUSH = False
def _want_flush(signum, frame):
    global _SHOULD_FLUSH
    _SHOULD_FLUSH = True

# register handlers (SLURM sends USR1 before timeout; TERM at the end)
signal.signal(signal.SIGUSR1, _want_flush)
signal.signal(signal.SIGTERM, _want_flush)

def _atomic_write_csv(df, path: Path):
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_suffix(path.suffix + ".tmp")
    df.to_csv(tmp, index=False)
    os.replace(tmp, path)  # atomic on POSIX

def _append_row_csv(row: dict, path: Path):
    import pandas as pd
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.exists():
        pd.DataFrame([row]).to_csv(path, mode="a", header=False, index=False)
    else:
        pd.DataFrame([row]).to_csv(path, mode="w", header=True, index=False)
...
def run_redistricting_analysis(
    ...
    # incremental outputs & checkpoint
    outdir = Path(output_dir)
    metrics_csv = outdir / "metrics_steps.csv"
    plans_csv   = outdir / "plans_long.csv"
    ckpt_json   = outdir / "progress.json"

    prog = _read_json(ckpt_json, {"last_step_saved": -1})
    last_saved = int(prog.get("last_step_saved", -1))
    start_write_at = (last_saved + 1) if resume else 0
    ...
    # small buffers for incremental writes
    metric_rows = []
    plan_rows = []
    ...
    def _flush(step_idx):
        if metric_rows:
            ...
            pd.DataFrame(metric_rows).to_csv(metrics_csv, mode="a", header=header, index=False)
            metric_rows.clear()
        if plan_rows:
            import pandas as pd
            header = not plans_csv.exists()
            pd.DataFrame(plan_rows).to_csv(plans_csv, mode="a", header=header, index=False)
            plan_rows.clear()
        _write_json({"last_step_saved": int(step_idx)}, ckpt_json)
    ...
    try:
        for i, partition in enumerate(chain):
            ...
            if i >= start_write_at:
                ...
                plan_rows.extend(
                    {"step": i, "precinct": n, "district": d}
                    for n, d in assign.items()
                )
            
                if (i % save_every_steps == 0) or _SHOULD_FLUSH or (i == num_steps - 1):
                    _flush(i)
                    if _SHOULD_FLUSH:
                        # SLURM is warning us; we already flushed
                        globals()["_SHOULD_FLUSH"] = False
...
def create_biased_ensemble(
    ...
    # write-each-map-now outputs + checkpoint
    outdir   = Path(output_dir)
    maps_dir = outdir / "maps"
    maps_dir.mkdir(parents=True, exist_ok=True)
    meta_csv = outdir / "ensemble_metrics.csv"
    ckpt_json = outdir / "progress.json"
    
    # resume by counting saved maps
    existing = sorted(maps_dir.glob("map_*.csv"))
    start_i = len(existing) if resume else 0
    if start_i >= int(ensemble_size):
        print(f"Nothing to do; {start_i} maps already saved.")
        return None, None
    ...
    for i in range(start_i, int(ensemble_size)):
        ...
        # ---- immediately persist this map & a one-line metric row ----
        assign = best_part.assignment
        df_assign = pd.DataFrame({"precinct": list(assign.keys()),
                                  "district": list(assign.values())})
        _atomic_write_csv(df_assign, maps_dir / f"map_{i:05d}.csv")
        ...
        _append_row_csv(meta_row, meta_csv)
        _write_json({"last_map_saved": i}, ckpt_json)
        
        # if SLURM warned us, bail cleanly after flushing this map
        if _SHOULD_FLUSH:
            globals()["_SHOULD_FLUSH"] = False
            break
