...
suppressPackageStartupMessages({
  ...
  library(ggplot2)
  # EI package:
  # install.packages("ei") # if needed
  library(ei)
})

# -------------------- CONFIG --------------------
prepared_dir = "Output/BLUEHIVE MAPS/EI_ER_prepared"           
progress_dir = file.path(prepared_dir, "progress")
dir.create(progress_dir, recursive = TRUE, showWarnings = FALSE)

# knobs for EI district-by-district
ei_burnin  = 5000
ei_sample  = 10000
...
# -------------------- HELPERS --------------------
# subset district assignments for a single plan (by plan_type + map_id)
get_district_assignments = function(assignments_long, plan_type, map_id) {
  df = assignments_long %>%
    filter(plan_type == tolower(plan_type), map_id == !!map_id) %>%
    select(precinct_id, district_id)
  if (nrow(df) == 0) stop("No assignments for plan_type=", plan_type, " map_id=", map_id)
  df
}

# precinct stats for EI/ER (merges in the district assignment)
calculate_precinct_stats = function(plan_type, map_id, precincts, assignments_long) {
  assign_df = get_district_assignments(assignments_long, plan_type, map_id)
  precinct_stats = precincts %>%
    inner_join(assign_df, by = "precinct_id") %>%
    mutate(plan_type = plan_type, map_id = map_id)
  
  # Add VAP aliases expected by EI code
...
run_ei_district = function(precinct_stats, ground_truth,
  ...
  run_one = function(dist_id) {
    ...
    res = try({
      ...
      # aggregate posterior draws to district level
      B = fit$draws$Beta
      n_draws = nrow(B)
      
      # columns are beta.{group}.{outcome}.{i}
      # minority dem columns:
      min_cols = paste0("beta.minority_vap.dem_votes.", seq_len(nrow(dat)))
      maj_cols = paste0("beta.majority_vap.dem_votes.", seq_len(nrow(dat)))
      
      # VAP weights
      w_min = dat$minority_vap
      w_maj = dat$majority_vap
      
      min_draws = maj_draws = numeric(n_draws)
      for (d in seq_len(n_draws)) {
        min_draws[d] = sum(as.numeric(B[d, min_cols]) * w_min) / sum(w_min)
        maj_draws[d] = sum(as.numeric(B[d, maj_cols]) * w_maj) / sum(w_maj)
      }
  ...
  if (parallel && requireNamespace("parallel", quietly = TRUE)) {
    ...
    parallel::clusterExport(
      cl,
      c("precinct_stats", "ground_truth", "burnin", "sample", "min_precincts"),
      envir = environment()
    )
...
# Progress cache
progress_path = function(plan_type, map_id) {
  file.path(progress_dir, paste0("map_", map_id, "_", plan_type, ".rds"))
}
...
for (ptype in plan_types) {
  ...
  pb = txtProgressBar(min = 0, max = length(maps), style = 3)
  k  = 0
  for (mid in maps) {
    k = k + 1
    if (is_done(ptype, mid)) { setTxtProgressBar(pb, k); next }
    
    # Build precinct stats + truth
