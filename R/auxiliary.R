...
create_population_data = function(n_precincts    = 2600,
  ...
  if (!is.null(input_data)) {
    ...
    # Generate total populations from log-normal (gives right-skewed population distribution)
    cv = pop_sd / pop_mean
    sigma = sqrt(log(1 + cv^2))
    mu = log(pop_mean) - sigma^2/2
    populations = round(rlnorm(n_precincts, meanlog = mu, sdlog = sigma))
    populations = pmax(50, populations)  # Minimum pop of 50
    
    # Generate minority percentages from Beta distribution
    # For exponential shape, alpha < 1 and beta > 1
    
    m = minority_mean
    v = minority_sd^2
    
    alpha = 0.8  # Keep < 1 
    beta = alpha * (1 - m) / m  # Adjust beta to maintain mean
    
    base_minority_pct = rbeta(n_precincts, alpha, beta)
    
    # Correlation with population size
    z_pop = scale(log(populations))
    ...
    # Apply correlation on logit scale
    logit_minority = logit(base_minority_pct) + rho_size_share * as.numeric(z_pop)
    minority_pct = invlogit(logit_minority)
    
    minority_pct = pmin(pmax(minority_pct, 0.01), 0.99)
  ...
  cat("\nGenerated", n_precincts, "precincts (Beta + log-normal approach)\n")
  ...
  cat("Max minority %:", round(max(precincts$per_minority) * 100, 2), "%\n")
  ...
  # 
  # # Population vs Minority Count scatter
  # plot(precincts$population, precincts$n_minority,
  #    main = "Population vs Minority Count",
  #    xlab = "Population", ylab = "Minority Count",
  #    pch = 16, col = rgb(1, 0, 0, 0.3))
  # abline(a = 0, b = minority_mean, lty = 2, col = "red")  # Expected relationship
  # 
  # par(mfrow = c(1, 1))
...
create_district_summary = function(map_data,
  ...
  # Align cd_plan to df$precinct_id if needed
  if (!is.null(plan_precinct_id)) {
    ord = match(df$precinct_id, plan_precinct_id)
    stopifnot(!any(is.na(ord)))
    stopifnot(length(cd_plan) >= length(ord))
    df$district_id = as.integer(as.vector(cd_plan[ord]))
  } else {
    stopifnot(length(cd_plan) >= nrow(df))
    df$district_id = as.integer(as.vector(cd_plan))
  }
  # Hard fail if anything went wrong
  if (anyNA(df$district_id)) stop("cd_plan produced NA district ids after coercion.")
  ...
  district_summary = df %>%
    ...
    dplyr::summarise(
      ...
      n_minority_voters  = sum(n_minority, na.rm = TRUE),
      n_majority_voters  = sum(n_majority, na.rm = TRUE),
      dem_votes          = sum(dem_votes, na.rm = TRUE),
      rep_votes          = sum(rep_votes, na.rm = TRUE),
      dem_votes_minority = sum(dem_votes_minority, na.rm = TRUE),
      dem_votes_majority = sum(dem_votes_majority, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    dplyr::mutate(
      per_minority           = safe_div(n_minority_voters, population),
      maj_min_district       = per_minority > 0.5,
      dem_voteshare          = safe_div(dem_votes, population),
      dem_district           = dem_voteshare >= 0.5,
      dem_voteshare_minority = safe_div(dem_votes_minority, n_minority_voters),
      dem_voteshare_majority = safe_div(dem_votes_majority, n_majority_voters),
      map_id                 = plan_id,
      district_id            = as.integer(district_id)
    ) %>%
    dplyr::mutate(
      dplyr::across(
        c(dem_voteshare_minority, dem_voteshare_majority),
        ~ ifelse(is.na(.), 0, .)
      )
    ) %>%
    dplyr::select(map_id, district_id, dplyr::everything())
...
save_plan_summaries = function(map_data, plan_data, plan_type, output_dir) {
  ...
  plan_columns = grep("^(step_\\d+|.*_map_\\d+)$", names(plan_data), value = TRUE) # plan columns only
...
district_lines = function(map_data, id = "district_id",
                           simplify_keep = 0.05,
                           min_len_m = 50) {
  stopifnot(id %in% names(map_data))
  ...
  # Boundaries → merged lines → extract LINESTRING parts
  lines = dpoly %>%
    sf::st_boundary() %>%
    sf::st_line_merge()
  
  lines = sf::st_collection_extract(lines, "LINESTRING", warn = FALSE)
...
plot_precincts_with_districts = function(map_data, CD_plans, map_ids, category_name, output_dir) {
  ...
  cd_borders_old = map_data %>%
    dplyr::group_by(district_id) %>%
    dplyr::summarise(geometry = sf::st_union(geometry), .groups="drop")
  table(sf::st_geometry_type(cd_borders_old))
  
  cd_lines = district_lines(map_data, id = "district_id",
                            simplify_keep = 0.05,   # small simplification is ok
                            min_len_m = 50)
  
  # 1) seat winners (no precinct outlines)
  p1 = ggplot(plot_df) +
    geom_sf(aes(fill = factor(dem_cd)), colour = NA, alpha = 0.9) +
    geom_sf(data = cd_lines, color = "darkred", linewidth = 0.8,
            lineend = "round", linejoin = "round", fill = NA) +
...
plot_winner_small = function(map_sf, assignment_vec, 
                             ...
                             precinct_outline_color = "#2b2b2b",
                             precinct_outline_width = 0.05) {
  ...
  vn = names(map_sf)
  if (!("precinct_id" %in% vn) && ("pct_id" %in% vn)) map_sf = dplyr::rename(map_sf, precinct_id = pct_id)
  if (!("population"  %in% vn) && ("pop"    %in% vn)) map_sf = dplyr::rename(map_sf, population  = pop)
  if (!("dem_votes"   %in% vn) && ("dem_v"  %in% vn)) map_sf = dplyr::rename(map_sf, dem_votes   = dem_v)
  if (!("rep_votes"   %in% vn) && ("rep_v"  %in% vn)) map_sf = dplyr::rename(map_sf, rep_votes   = rep_v)
  ...
  # District polygons (union precincts by district), then join winners
  cd_polys = map_sf %>%
    dplyr::group_by(district_id) %>%
    dplyr::summarise(geometry = sf::st_union(geometry), .groups = "drop") %>%
  ...
  p = ggplot() +
    # 1) Fill districts by winner
    geom_sf(data = cd_polys, aes(fill = party), color = NA) +
    # 2) Precinct outlines (thin)
    geom_sf(data = map_sf, fill = NA, color = precinct_outline_color, linewidth = precinct_outline_width) +
    # 3) District outline (thick)
...
analyze_redistricting_impact = function(output_base_dir      = "Output/Tests/",
  ...
  for (current_level in segregation_levels) {
    ...
    # Save for cross-level plotting
    results_for_comp_plots[[current_level]] = list(precincts = precincts_baseline)
    
    # Format (these are the votes the shapefile will carry through Python)
    map_data_formatted = prepare_for_redistricting(precincts_baseline)
    ...
    # Initial figs
    create_initial_visualizations(precincts_baseline, map_data_formatted, current_output_dir, current_level)
    ...
    tryCatch({
      ...
      # Make baseline figures (using the exact votes used to place precincts)
      if (!is.null(baseline_summaries)) {
      ...
      # Optionally run ex-post vote models to re-score the SAME plans
      if (isTRUE(score_models)) {
        # Provide models as a named list:
        # Sensible default:
        if (!exists("expost_vote_models") || is.null(expost_vote_models)) {
          expost_vote_models = list(
            ...
            contextual = list(
              ...
              shared_field_wt   = shared_field_wt,
              shared_field_rho  = shared_field_rho,
              seed              = random_seed + 123  # keep fixed across levels if you want comparability
            )
          )
        }
        
        cat("\nRe-scoring ensembles under EX-POST vote model(s)...\n")
        base_sf_for_models = precincts_baseline %>% 
          dplyr::select(precinct_id, population, n_minority, n_majority, per_minority, geometry)
        
        model_summaries = list()
        for (vm_name in names(expost_vote_models)) {
          vm_params = expost_vote_models[[vm_name]]
          model_summaries[[vm_name]] = evaluate_vote_model_on_fixed_plans(
            level_dir         = current_output_dir,
            base_precincts_sf = base_sf_for_models,
            vote_model_name   = vm_name,
            vm_params         = vm_params
          )
        }
        
        # Create figures/visualizations for ex-post vote models here
      }
