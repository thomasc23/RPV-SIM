...
def _normalize_gdf_for_chain(gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    if "n_min" in gdf.columns and "n_maj" in gdf.columns:
...
def save_neutral_results(metrics, CD_assignments, gdf, output_dir):
    ...
    if num_steps_completed > 0:
        ...
        # Carry through vendor id if present
...
def create_biased_ensemble(
    ...
    dev_mode=False,
    burst_length=50,           # steps per burst
    num_bursts=10,             # total bursts
    patience_bursts=5,         # early stop after this many bursts w/o improvement
    # --- Geometry speed knob ---
    ...
    # Replace NAs with imputed mean
    gdf["dem_vsh_1"] = gdf["dem_vsh_1"].fillna(gdf["dem_vsh_1"].mean())
    ...
    # Average within precinct variance (induced by binomial sampling of betab, betaw)
    gdf["within_pct_var"] = (
        gdf["n_minority"] * gdf["dem_vsh_1"] * (1 - gdf["dem_vsh_1"]) +
        gdf["n_majority"] * gdf["dem_vsh_0"] * (1 - gdf["dem_vsh_0"])
    )

    # Sum of party R win probabilities
    def repub_win_prob(p):
        num_dists = len(p.parts)
        win_prob = [0] * num_dists

        for i in range(int(num_dists)):
            # Filter precincts within each partition (congressional district)
            filtered = gdf[gdf["precinct_id"].isin(p.parts[i])]

            # Mean vote share
            mean_dem_sh = sum(filtered.dem_v) / (sum(filtered.dem_v) + sum(filtered.rep_v))

            # Calculate average cross precinct variance
            n_min = filtered["n_minority"].astype(int)
            n_maj = filtered["n_majority"].astype(int)
            betab = filtered["dem_vsh_1"].astype(float)
            betaw = filtered["dem_vsh_0"].astype(float)
            cross_pct_var = sum(
                n_min * n_min * betab.var() +
                n_maj * n_maj * betaw.var() +
                2 * n_min * n_maj * betab.cov(betaw)
            )
            within_pct_var = sum(filtered["within_pct_var"])

            # Variance of the proportion of Democratic voters
            pop_squared = sum(filtered.population) ** 2
            var_dem_sh = (cross_pct_var + within_pct_var) / float(pop_squared)

            # Win probability
            win_prob[i] = 1 - normal_cdf(mean_dem_sh - 0.5, 0, stats.sqrt(var_dem_sh))
        return win_prob
    ...
    def exp_seats_R(p):
        win_prob = repub_win_prob(p=p)
        return sum(x > 0.5 for x in win_prob)

    def exp_seats_D(p):
        return num_districts - exp_seats_R(p)

    # Soft surrogate for seat count (logistic around 50%)
    def soft_seat_score(part, party="republican", k=soft_k):
        shares_R = np.array(repub_win_prob(p=part))
        if party == "democratic":
            shares_D = 1.0 - shares_R
    ...
    if use_soft_score:
        ...
        metric_func = exp_seats_D if bias_type == "democratic" else exp_seats_R
    ...
    for i in range(int(ensemble_size)):
        ...
        print(f"bias_type={bias_type} | init seats -> D_obs={seats_D(initial)}, D_exp={exp_seats_D(initial)}")
            ...
            for part in optimizer.short_bursts(burst_length=int(burst_length), num_bursts=int(num_bursts)):
                ...
                if step_total == 1 or step_total % 250 == 0 or step_total % int(burst_length) == 0:
                    print(f"  step {step_total:5d}: best={optimizer.best_score:.3f} | "
                          f"curr D_obs={seats_D(part)}, D_exp={exp_seats_D(part)}")
        ...
        metrics_records.append({
            ...
            "exp_dem_seats": exp_seats_D(best_part),
